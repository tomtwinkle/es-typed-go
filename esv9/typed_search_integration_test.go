...
func createTypedSearchIndex(t *testing.T, ctx context.Context, client esv9.ESClient, idx estype.Index, alias estype.Alias) {
	t.Helper()
...
func indexTypedSearchDocs(t *testing.T, ctx context.Context, client esv9.ESClient, alias estype.Alias, docs []typedSearchDoc) {
	t.Helper()
...
func TestIntegration_TypedSearch_RequestOptions(t *testing.T) {
	...
	t.Run("typed params zero-sized search does not return parse error", func(t *testing.T) {
		...
		timeout := "30s"
		res, err := esv9.Search[typedSearchDoc](ctx, client, alias, esv9.SearchParams{
	...
	t.Run("builder search_after request does not return parse error", func(t *testing.T) {
		...
		rawRes, err := client.SearchRaw(ctx, alias, params.ToV9Request())
		assert.NilError(t, err)
	...
	t.Run("typed search exposes hit sort values for search_after pagination", func(t *testing.T) {
		...
		page1, err := esv9.Search[typedSearchDoc](ctx, client, alias, esv9.SearchParams{
			Query:          query.TermValue(typedSearchFieldCategory, "kitchen"),
			Sort:           sorts,
			Size:           1,
			HasSize:        true,
		...
		page2, err := esv9.Search[typedSearchDoc](ctx, client, alias, esv9.SearchParams{
			Query:          query.TermValue(typedSearchFieldCategory, "kitchen"),
			Sort:           sorts,
			Size:           1,
			HasSize:        true,
