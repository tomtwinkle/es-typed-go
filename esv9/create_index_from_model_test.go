...
func TestToTypedIndexSettings_AllFields(t *testing.T) {
	...
	refreshInterval, ok := got.RefreshInterval.(*string)
	assert.Assert(t, ok)
	assert.Equal(t, "1s", *refreshInterval)
}
...
func TestToTypedIndexSettings_DisabledRefresh(t *testing.T) {
	...
	refreshInterval, ok := got.RefreshInterval.(*string)
	assert.Assert(t, ok)
	assert.Equal(t, "-1", *refreshInterval)
}
...
func TestToTypedTypeMapping_LeafProperties(t *testing.T) {
	...
	assert.Assert(t, ok)
	_, ok = titleProp.(es9types.TextProperty)
	assert.Assert(t, ok)

	priceProp, ok := got.Properties["price"]
	assert.Assert(t, ok)
	_, ok = priceProp.(es9types.IntegerNumberProperty)
	assert.Assert(t, ok)

	dateProp, ok := got.Properties["date"]
	assert.Assert(t, ok)
	_, ok = dateProp.(es9types.DateProperty)
	assert.Assert(t, ok)
}
...
func TestToTypedTypeMapping_TextMultiField(t *testing.T) {
	...
	titleProp, ok := got.Properties["title"]
	assert.Assert(t, ok)
	textProp, ok := titleProp.(es9types.TextProperty)
	assert.Assert(t, ok)
	assert.Assert(t, textProp.Fields != nil)

	keywordField, ok := textProp.Fields["keyword"]
	assert.Assert(t, ok)
	keywordProp, ok := keywordField.(es9types.KeywordProperty)
	assert.Assert(t, ok)
...
func TestToTypedTypeMapping_NestedProperties(t *testing.T) {
	...
	assert.Assert(t, ok)
	_, ok = valueProp.(es9types.IntegerNumberProperty)
	assert.Assert(t, ok)
}
...
func TestToTypedTypeMapping_FieldTypeFallbacks(t *testing.T) {
	...
	_, ok := got.Properties["status"].(es9types.KeywordProperty)
	assert.Assert(t, ok)
	_, ok = got.Properties["title"].(es9types.TextProperty)
	assert.Assert(t, ok)
	_, ok = got.Properties["price"].(es9types.IntegerNumberProperty)
	assert.Assert(t, ok)
	_, ok = got.Properties["date"].(es9types.DateProperty)
	assert.Assert(t, ok)
	_, ok = got.Properties["items"].(es9types.NestedProperty)
	assert.Assert(t, ok)
}
...
func TestCreateIndexModelStub_ProducesConvertibleSettingsAndMapping(t *testing.T) {
	...
	refreshInterval, ok := settings.RefreshInterval.(*string)
	assert.Assert(t, ok)
	assert.Equal(t, "1s", *refreshInterval)
	...
	keywordField, ok := textProp.Fields["keyword"]
	assert.Assert(t, ok)
	_, ok = keywordField.(es9types.KeywordProperty)
	assert.Assert(t, ok)
