...
func TestIntegration_Search_WithSorting(t *testing.T) {
	...
	for _, hit := range res.Hits.Hits {
		var got productDoc
		assert.NilError(t, json.Unmarshal(hit.Source_, &got))
		assert.Assert(t, got.Price >= prev)
		prev = got.Price
	}
}
...
func createNestedSortIndex(t *testing.T, ctx context.Context, client esv9.ESClient, idx estype.Index, alias estype.Alias) {
	t.Helper()
	mappings := &types.TypeMapping{
		Properties: map[string]types.Property{
			"name":  types.NewKeywordProperty(),
