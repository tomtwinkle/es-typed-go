...
func (c *esClient) CatCircuitBreaker(ctx context.Context, opts ...CatCircuitBreakerOption) (cat_circuit_breaker.Response, error) {
	r := c.typedClient.Cat.CircuitBreaker()
	for _, opt := range opts {
		opt(r)
	}
	return r.Do(ctx)
